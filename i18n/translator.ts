...
/**
 * On-the-fly translator backed by MyMemory's free public endpoint (no API key,
 * ~5k chars/day/IP). Each successful translation is persisted in AsyncStorage
 * under the `i18n.cache.` prefix so subsequent app launches and re-renders of
 * the same source string are instant and offline-safe.
 *
 * Production upgrade path: replace the endpoint with a paid provider
 * (Google / DeepL / Azure), pre-translate static strings at build time, and
...
export async function translate(text: string, target: string): Promise<string> {
  ...
  try {
    ...
    // storage failures fall through to network
  }
